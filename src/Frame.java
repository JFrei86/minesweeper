 ...
 * Frame inherits from JFrame and contains JMenu and JMenuItem components
 * for the GUI and the panel in which the board lays it's JButtons.
 * Private data includes a boolean that indicates if the player has 
 * clicked on the board for the first time, the number of buttons,
 * the number of flags, the Board object taken by the constructor,
 * the GridLayout of the board panel, etc.
 */
public class Frame extends JFrame{
	/**
	 * Private Data:
	 * timer - keeps track of how long the user has been playing and updates
	 * 		the time from the time the player first clicks to their winning 
	 * 		or losing state
	 * layout - keeps the gameBoard in the rows and columns represented in
	 * 		the Board
	 * gameBoard - the JPanel that contains the JButtons from the Board's 
	 * 		Blocks
	 * board - the Board taken by the Contructor's argument
	 * numButtons - the number of JButtons in the panel. When initialized,
	 * 		this is the width * height
	 * numFlags - the number of flagged squares on the board. When initialized,
	 * 		this is zero.
	 * isFirstClick - the status of the board. Used to determine if the user 
	 * 		has made their first move. Used to ensure that the user cannot lose 
	 * 		on the first click.
	 * easy, med, hard, custom, newGame - radio buttons that are components of 
	 * 		the menu at the top of the GUI.
	 * minesRemaining - shows the current number of flags and the distance from winning
	 */
	...
	private JRadioButtonMenuItem easy = new JRadioButtonMenuItem("Easy (9 x 9, 10 Mines)");
	private JRadioButtonMenuItem med = new JRadioButtonMenuItem("Medium (16 x 16, 40 Mines)");
	private JRadioButtonMenuItem hard = new JRadioButtonMenuItem("Hard (16 x 30, 99 Mines)");
	 ...
	 * @param b the Board object that is to be used upon initializing the game.
	 */
	...
	public void reset(){
		...
		for(int i = 0; i < board.getWidth(); i++){
			for(int j = 0; j < board.getHeight(); j++){
				...
				board.getBlock(i, j).getButton().addMouseListener(new GameListener(i,j));
	...
	public void endGame(boolean aFlag){
		...
		if(!aFlag){
			for(int i = 0; i < board.getWidth(); i++){
				for(int j = 0; j < board.getHeight(); j++){
					if(board.getBlock(i, j).getNumMines() == -1 && !board.getBlock(i, j).isClicked() && !board.getBlock(i, j).isFlagged()){
						click(i,j,board.getBlock(i,j).getNumMines());
					}
					if(board.getBlock(i, j).getNumMines() != -1 && board.getBlock(i, j).isFlagged()){
			...
			for(int i = 0; i < board.getWidth(); i++){
				for(int j = 0; j < board.getHeight(); j++){
					if(board.getBlock(i, j).getNumMines() == -1 && !board.getBlock(i, j).isClicked()){
		...
		if(board.getBlock(y, x).getNumMines() == 0)
			for(int i = y - 1; i < y + 2; i++){
				for(int j = x - 1; j < x + 2; j++){
					if(board.isLegal(i, j) && board.getBlock(i, j).getNumMines() != -1 && !board.getBlock(i, j).isClicked()){
						gameClick(i, j, board.getBlock(i, j).getNumMines());
						zeroClick(i, j);
					}
				}
			}
	...
	public void flagClick(int y, int x){
		if(board.getBlock(y, x).getNumMines() == board.getNeighborFlags(y, x)){
			for(int i = y - 1; i < y + 2; i++){
				for(int j = x - 1; j < x + 2; j++){
					if(board.isLegal(i, j) && !board.getBlock(i, j).isFlagged() && !board.getBlock(i, j).isClicked()){
						gameClick(i, j, board.getBlock(i, j).getNumMines());
					}
				}
			}
	 ...
	 * @param mines
	 */
	public void gameClick(int y, int x, int mines){
		click(y, x, mines);
		if(board.getBlock(y, x).getNumMines() == -1){
			gameBoard.getComponents()[y * board.getHeight() + x].setBackground(Color.RED);
			endGame(false);
			return;
		}
	...
	public void toggleFlag(int y, int x){
		if(!board.getBlock(y, x).isFlagged()){
			...
			board.getBlock(y, x).getButton().setIcon(new MineImage(MineImage.FLAG));
			updateProgressBar(++numFlags);
	...
	private class MenuListener implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getSource() == custom){
				int y = getInt("Rows: Please Enter a Number between 1 and 30", 30);
				int x = getInt("Columns: Please Enter a Number between 1 and 30", 30);
				int max = y * x / 2;
				newGame(y, x, getInt("Mines: Please Enter a Number between 1 and " + max, max));
				easy.setSelected(false);
				med.setSelected(false);
				hard.setSelected(false);
			}
	...
	private class GameListener implements MouseListener{
		...
		public void mousePressed(MouseEvent arg0) {
			...
			if(board.getBlock(y, x).isFlagged() || !board.getBlock(y, x).getButton().isEnabled())
				return;
			if(isFirstClick){
				firstClick(y, x);
			}
			gameClick(y, x, board.getBlock(y, x).getNumMines());
...
/* TODO
 * fix custom game feature if time permits
 */
